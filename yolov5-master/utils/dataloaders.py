...
class LoadScreenshots:
    ...
    def __next__(self):
        ...
        while True:
            try:
                if self.im_files[self.count].endswith('.npy'):
                    im0 = np.load(self.im_files[self.count])
                    s = f"image {self.count}/{self.nf} {self.im_files[self.count]}: "
                else:
                    #im0 = cv2.imread(self.im_files[self.count])  # BGR
                    #im0 = np.load(self.im_files[self.count])  # BGR
                    s = f"image {self.count}/{self.nf} {self.im_files[self.count]}: "
            except Exception as e:
                LOGGER.warning(f'{prefix}WARNING: Ignoring corrupt image/label at {self.im_files[self.count]}: {e}')
                self.count += 1
                continue
            ...
            self.count += 1
            return self.im_files[self.count - 1], im, im0, self.cap, s
...
def extract_boxes(path=DATASETS_DIR / "coco128"):
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in IMG_FORMATS:
            ...
            im = np.load(str(im_file))[..., ::-1]  # BGR to RGB
            h, w = im.shape[:2]
...
def verify_image_label(args):
    ...
    try:
        ...
        if im.format.lower() in ("jpg", "jpeg", "npy"):
